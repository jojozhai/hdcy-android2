...
public class BGABanner extends RelativeLayout implements BGAViewPager.AutoPlayDelegate, ViewPager.OnPageChangeListener {
    ...
    private int mTipGravity = Gravity.CENTER_HORIZONTAL|Gravity.CENTER;
    ...
    private void initCustomAttr(int attr, TypedArray typedArray) {
        ...
        }else if(attr== R.styleable.BGABanner_banner_mtipsGravity){
            mTipGravity =typedArray.getInt(attr,mTipGravity);
        }
    ...
    private void initView(Context context) {
        ...
        RelativeLayout TextContainerRl = new RelativeLayout(context);
        if (Build.VERSION.SDK_INT >= 16) {
            TextContainerRl.setBackground(mPointContainerBackgroundDrawable);
        } else {
            TextContainerRl.setBackgroundDrawable(mPointContainerBackgroundDrawable);
        }
        TextContainerRl.setPadding(mPointContainerLeftRightPadding, mPointTopBottomMargin, mPointContainerLeftRightPadding, mPointTopBottomMargin);
        TextContainerRl.setGravity(Gravity.CENTER);
        ...
        RelativeLayout.LayoutParams textContainerLp = new RelativeLayout.LayoutParams(RMP,RWC);
        //处理 文字在顶部还是底部
        textContainerLp.setMargins(0,200,0,0);
        textContainerLp.addRule(RelativeLayout.ALIGN_PARENT_TOP);
        addView(TextContainerRl, textContainerLp);

        RelativeLayout.LayoutParams tipLp = new RelativeLayout.LayoutParams(RMP, RWC);
        tipLp.addRule(RelativeLayout.CENTER_IN_PARENT);
        mTipTv = new TextView(context);
        mTipTv.setGravity(Gravity.CENTER_HORIZONTAL);
        mTipTv.setSingleLine(true);
        mTipTv.setEllipsize(TextUtils.TruncateAt.END);
        mTipTv.setTextColor(mTipTextColor);
        mTipTv.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTipTextSize);
        TextContainerRl.addView(mTipTv, tipLp);
    ...
    private void initViewPager() {
        ...
        ViewPager.LayoutParams viewpagercontainer = new ViewPager.LayoutParams();
        viewpagercontainer.height =RMP;
        mViewPager.setLayoutParams(viewpagercontainer);
        //
        //mViewPager.setLayoutParams();
        FrameLayout.LayoutParams haha = new FrameLayout.LayoutParams(RMP,RMP);


        addView(mViewPager, 0, new FrameLayout.LayoutParams(RMP, RMP));
